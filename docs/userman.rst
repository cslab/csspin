...
Here are two examples from a previous version of the spin project
itself.
...
the configuration from the ``@windows`` key in :file:`global.yaml`.


Cruises are selected by using the :option:`-c <spin --cruise>` (or
:option:`--cruise <spin --cruise>`) option to :program:`spin`. The
following will run spin in the Linux container.
...
A special selector is ``@all``, selecting all cruises. In the example
above, this means running the requested task for all supported
platforms.
